...
class ReentrantNetworkEnv(gymnasium.Env):
    def __init__(self, distance_matrix, buffer_capacity_1 = 250, buffer_capacity_6 = 1, buffer_capacity=10, num_buffers=10, failure_prob=0.01, max_time_steps=1000):
    ...
    def step(self, action):
        ...
        if action == 5:
            if 0 < len(self.buffer[action - 1]) and len(self.buffer[action]) < self.buffer_capacity_6 and len(self.machines_busy[action-1]) < 1: # extra if statement to check if 6th buffer is not full
        ...
        elif 0 < action < self.num_buffers and action != 5: # if any action but last buffer is chosen
...
#axes[0].axhline(y=50, color='purple', linestyle=':', linewidth=2, label=f'Buffer Capacity 1 (50)')
