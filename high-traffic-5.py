...
class ReentrantNetworkEnv(gymnasium.Env):
    ...
    def _package_arrivals(self):
        ...
        new_packages = np.random.poisson(0.8) # arrival rate of packages
        ...
        virtual_arrival = np.random.poisson(0.4) # simulate virtual arrivals to wait buffer
...
fig.suptitle('High Traffic')
fig.tight_layout(pad=3.0)
axes[0].plot(average_rewards_per_episode)
axes[0].set_xlabel('Episodes')
axes[0].set_ylabel('Average Amount of Packages')
#axes[0].axhline(y=150, color='purple', linestyle=':', linewidth=2, label=f'Buffer Capacity 1 (150)')
