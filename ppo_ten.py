...
def evaluate_model(model, env,
                   max_iterations = 200, max_steps_per_iteration = 10000):
    random.seed(234895)
    np.random.seed(234895)
    tf.random.set_seed(234895)
    print('Evaluation started')

    average_rewards_per_iteration_det = []
    ...
    for iteration in range(max_iterations):
        ...
        average_rewards_per_iteration_det.append(average_rewards_det)
        total_packages_left_det.append(env.packages_left)

    fig, axes = plt.subplots(1, 2)
    fig.suptitle('Deterministic Policy')
    fig.tight_layout(pad=3.0)
    axes[0].plot(average_rewards_per_iteration_det)
