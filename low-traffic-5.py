...
class ReentrantNetworkEnv(gymnasium.Env):
    def __init__(self, distance_matrix, buffer_capacity_1 = 250, buffer_capacity=10, num_buffers=5, failure_prob=0.01, max_time_steps=1000):
    ...
    def _package_arrivals(self):
        ...
        virtual_arrival = np.random.poisson(0.05) # simulate virtual arrivals to wait buffer
    ...
    def _service_time(self, buffer_index):
        ...
        service_time = float(
            stochastic_time + self.robot_distances[buffer_index])  # Total Service time is processing time plus transportation time
...
fig.suptitle('Low Traffic')
fig.tight_layout(pad=3.0)
axes[0].plot(average_rewards_per_episode)
axes[0].set_xlabel('Episodes')
axes[0].set_ylabel('Average Amount of Packages')
#axes[0].axhline(y=20, color='purple', linestyle=':', linewidth=2, label=f'Buffer Capacity 1 (20)')
